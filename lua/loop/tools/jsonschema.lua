...
local function validate(schema, data, path)
  ...
  -- 1. Type check — supports type as string or array (e.g. ["string", "null"])
    ...
    if addl == false then
      for key in pairs(data) do
        if not props[key] then
          ...
          add_error(errors, join_path(path, key), "invalid property, allowed: " .. (valid ~= "" and valid or "(none)"))
        end
      end
  ...
  if schema.type == "array" or (type(schema.type) == "table" and vim.tbl_contains(schema.type, "array")) then
    ...
    if schema.items then
      for i, value in ipairs(data) do
        local sub_err = validate(schema.items, value, path .. "/" .. (i - 1))
        if sub_err then
          vim.list_extend(errors, sub_err)
        end
      end
