...
function BaseSession:_handle_event(msg)
    ...
    local cb_error = function(err)
        self.log:error("In event handler for " ..
...
function BaseSession:_handle_resp(msg)
    ...
    local cb_error = function(err)
        self.log:error("In response handler for " ..
        (msg.command or "?") .. "\n" .. debug.traceback("Error: " .. tostring(err) .. "\n", 2))
    end
...
function BaseSession:_handle_rev_req(msg)
    ...
    local cb_error = function(err)
        self.log:error("In rev-request handler for " ..
        (msg.command or "?") .. "\n" .. debug.traceback("Error: " .. tostring(err) .. "\n", 2))
    end
...
-- ===================================================================
-- Request methods — ONLY annotations changed, code 100% identical
-- ===================================================================
...
-- Remaining rarely-used / adapter-specific requests stay as table
---@param args table
---@param callback fun(response: loop.dap.proto.Response)|nil
