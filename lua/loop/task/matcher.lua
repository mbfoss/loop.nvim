...
---@return loop.task.TaskIssue|nil
local function _make_issue(m, line)
...
---@return loop.task.TaskIssue[]|nil
function _get_issues(lines, matcher)
    local arr = {}
    for _, line in ipairs(lines) do
        line = line:gsub("[\r]+$", "")
        line = strip_ansi_codes(line)
        local e = _make_issue(matcher, line)
        if e then
            table.insert(arr, e)
        end
    end
...
---@return loop.task.TaskIssue[]|nil
function M.get_issues(lines, matcher)
    ------------------------------------------------------------------
    ...
    if type(matcher) == "table" and matcher.regexp then
        return _get_issues(lines, matcher)
    end
    return nil
end
