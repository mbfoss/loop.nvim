...
---@class loop.pages.DebugPage : loop.pages.Page
---@field new fun(self: loop.pages.DebugPage, filetype: string, on_buf_enter: fun(buf: integer)): loop.pages.DebugPage
local DebugPage = class(Page)

-- Static namespace for extmarks
local NS_ID = vim.api.nvim_create_namespace('loop-debug-hl')
...
function DebugPage:init(filetype, on_buf_enter)
	Page.init(self, filetype, on_buf_enter)
	self._items = {}
end

function DebugPage:get_buf()
...
---@return number
function DebugPage:_get_curr_row()
	local buf = self.buf
...
function DebugPage:_refresh_buffer()
	local buf = self.buf
	if not buf or not vim.api.nvim_buf_is_valid(buf) then
		return
	end
...
function DebugPage:setlist(items, proj_dir)
...
function DebugPage:get_selected()
